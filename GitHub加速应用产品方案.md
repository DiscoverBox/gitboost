...
> 一个面向 GitHub 公开 HTTPS 仓库的跨平台桌面加速工具。用户继续使用原始 `https://github.com/...` 地址，应用把 Git 的读取地址临时重写到本机检测并选中的外部加速节点。

MVP 使用“静态系统节点目录 + 本地数据面”：

- 官方只发布一个由代理 URL 字符串组成的静态 JSON，不建设数据库、在线 API、爬虫、社区提交或管理后台。
- 不使用 SQLite 或其他数据库。
- 不运行本地 HTTP 转发服务，不代理 Git 数据流。
- 客户端缓存系统节点目录，并与用户手工添加或从本地 JSON 导入的自定义节点合并。
- 应用负责节点校验、测速、选择以及 Git 配置的安全写入与恢复。
- 开发按可验收交付批次执行，不采用传统团队的周数排期。

这一版本验证的是最核心假设：

> 用户是否愿意安装一个不需要手工维护系统线路的桌面工具，在不改变日常 clone 命令的前提下获得更稳定的公开仓库下载体验。
...
- 批量粘贴外部节点。
- 导入本地节点 JSON 文件。
- 从官方静态 URL 数组更新系统节点，并在本地保留最后一次有效缓存。
- 节点增删、启用、禁用和重新命名。
- 使用真实 `git ls-remote` 校验 Git Smart HTTP 能力。
- 对可用节点进行多次测试，按成功率和耗时排序。
- 自动选择、GitHub 直连两种线路状态。
- 选择自动线路即开启加速，选择 GitHub 直连即关闭加速，不保留相互矛盾的中间状态。
- 全局加速、仅加速清单两种路由范围。
...
- 希望开箱即用，同时允许补充自有 GitHub 加速节点的用户。
...
6. 应用询问用户是否使用私有仓库，用户选择路由范围并按需配置公开仓库清单。
7. 用户选择“自动选择”或点击“开启加速”，两种入口执行相同操作。
8. 应用选择可用节点并生成独立 gitconfig。
9. 应用注册一次全局 `include.path`。
...
- 应用没有运行时不会自动发现节点故障。
- 自动模式下，当前 `clone`、`fetch`、`pull` 或 `ls-remote` 失败后，应用复检实际使用的当前节点；确认节点异常后自动为下一次 Git 操作换线，用户仍需重试原命令。
- GitHub 直连、其他 URL 重写和 `push` 失败不会触发自动换线；关闭使用日志不影响自动故障复检。
...
- 批量粘贴、导入 JSON。
- 只展示用户添加的节点、名称和规范化地址；系统节点由远程目录和本地缓存维护。
...
全量检测最多同时运行 4 个节点，并且启动刷新、周期健康检查和用户手动检测共用同一个全量任务互斥状态，避免重复检测和健康结果互相覆盖。
...
- `nodes.json`：用户添加或导入的自定义节点。
- `system-nodes.json`：官方 URL 数组最近一次通过校验的本地缓存。
...
| 是否仍包含动态节点 API、爬虫、签名和管理后台 | 通过：不包含 |
| 系统节点是否只是静态 URL 数组 | 通过 |
| 是否仍包含本地转发端口 | 通过：不包含 |
...
2. 系统节点来自官方静态目录，自定义节点由用户添加；两者都必须通过本机真实 Git 检测。
3. 混合使用私有仓库的用户默认采用“仅加速清单”。
4. 节点失效后的恢复方式是“选择新节点并重试”，不是当前命令无感续传。
5. MVP 的成败应先由真实用户流程和跨平台兼容性决定，再考虑自动发现、目录签名或其他后台能力。
